...
{  
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            SLAM.TrackMonocular(im, tframe, vector<ORB_SLAM3::IMU::Point>(), vstrImageFilenames[seq][ni]); // TODO change to monocular_inertial
    ...
    if (bFileName)
    {
        ...
        const string pc_file =  "pc_" + string(argv[argc-1]) + ".json";
        SLAM.SaveTrajectoryEuRoC(f_file);
        SLAM.SaveKeyFrameTrajectoryEuRoC(kf_file);
        SLAM.SavePointCloudJSON(pc_file);
    }
    else
    {
        SLAM.SaveTrajectoryEuRoC("CameraTrajectory.txt");
        SLAM.SaveKeyFrameTrajectoryEuRoC("KeyFrameTrajectory.txt");
        SLAM.SavePointCloudJSON("PointCloud.json");
    }
