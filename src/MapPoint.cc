...
void MapPoint::IncreaseTreeDetectionCount(int n)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mTreeDetectionCount += n;
}

int MapPoint::GetTreeDetectionCount()
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mTreeDetectionCount;
}

bool MapPoint::IsTreePoint(int threshold)
{
    unique_lock<mutex> lock(mMutexFeatures);
